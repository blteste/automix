...
app.get('/api/users', async (req, res) => {
  ...
  const pageNumber = parseInt(page, 10) > 0 ? parseInt(page, 10) : 1; // Garante que page seja um número válido
  const pageLimit = parseInt(limit, 10) > 0 ? parseInt(limit, 10) : 9; // Garante que limit seja um número válido

  try {
    // Verifica o token
    const decoded = jwt.verify(token, SECRET_KEY);

    // Busca os Utilizador com paginação
    const users = await prisma.user.findMany({
      take: pageLimit,
      skip: (pageNumber - 1) * pageLimit,
    });

    // Conta o número total de Utilizador
