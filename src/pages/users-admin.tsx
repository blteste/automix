...
const Utilizadores = () => {
  ...
  const handlePageChange = async (page: number) => {
    ...
    setCurrentPage(page); // Atualiza a página
    await fetchUsers(page); // Chama a função que busca os dados da API
  
    // Verifique se a renderização está sendo feita corretamente com os dados mais recentes.
  };
  ...
  <h2 className="text-3xl font-extrabold mb-6 text-center text-[#0022FF]">Lista de Utilizadores</h2>
  ...
  <Pagination className="mt-6">
    <PaginationContent>
      ...
      <PaginationItem>
        <PaginationPrevious
          onClick={handlePreviousPage} // Chama a função de navegação para a página anterior
          disabled={currentPage === 1} // Desativa o botão se a página atual for 1
        >
          Anterior
        </PaginationPrevious>
      </PaginationItem>
      ...
      {[...Array(totalPages)].map((_, index) => (
        <PaginationItem key={index + 1}>
          <PaginationLink
            onClick={() => handlePageChange(index + 1)} // Chama a função de navegação para a página selecionada
            isActive={index + 1 === currentPage} // Marca a página atual como ativa
          >
            {index + 1}
          </PaginationLink>
        </PaginationItem>
      ...
      <PaginationItem>
        <PaginationNext
          onClick={handleNextPage} // Chama a função de navegação para a próxima página
          disabled={currentPage === totalPages} // Desativa o botão se a página atual for a última
        >
          Próximo
        </PaginationNext>
      </PaginationItem>
