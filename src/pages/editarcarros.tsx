...
const EditCarPage = () => {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    accept: ".jpg,.jpeg,.png", // Tipos de arquivo aceitos
    onDrop: (acceptedFiles) => {
      // Manter as imagens existentes e adicionar as novas imagens
      setImages((prevImages) => {
        const newImages = Array.from(acceptedFiles);
        return [...(prevImages || []), ...newImages]; // Usando "newImages" ao invés de "newImage"
      });
    },
  });
...
>
  ...
  {brand && marcasModelos[brand] && marcasModelos[brand].length > 0 ? (
    ...
    <option disabled>Nenhum modelo disponível</option>
            ...
            {/* Additional Form Fields (Year, Price, etc.) */}
            <div className="flex flex-col">
